...
DiVoPostTCR 在 DiVo 管线中的定位是**支线待验证技术**——保留权重和推理引擎，但不进入主管线。主管线使用 DeepTCR + NetTCR + ERGO-II 三工具交叉验证体系（全 CPU，不依赖 GPU）。

这样设计的原因：交叉验证体系用三个完全不同架构（VAE/CNN/LSTM）的独立工具互相印证，科学上比依赖单一模型更可靠。而 DiVoPostTCR 作为最有潜力的"第四意见"，待数据偏倚验证通过后再考虑接入。
